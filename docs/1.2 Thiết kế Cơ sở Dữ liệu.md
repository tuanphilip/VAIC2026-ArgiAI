...
Tài liệu này đặc tả cấu trúc cơ sở dữ liệu quan hệ (PostgreSQL) phục vụ cho dự án ArgiAI, giúp hệ thống lưu trữ thông tin người dùng, mùa vụ, nhật ký bệnh hại, dự báo năng suất và giá cả thị trường.
...
erDiagram
    ...
    CROPS {
        uuid id PK
        string name "Lúa Điện Biên, Cà phê Mường Ảng..."
        string variety "Giống cụ thể"
    ...
    DISEASE_LOGS {
        ...
        uuid reporter_id FK "Liên kết với USERS"
        string image_url "Đường dẫn lưu trữ ảnh"
        string detected_disease "Tên bệnh do AI phát hiện"
        float confidence "Độ tin cậy của mô hình (0.0 -> 1.0)"
        text treatment_measures "Biện pháp điều trị"
    ...
    MARKET_PRICES {
        uuid id PK
        uuid crop_id FK "Liên kết với CROPS"
        float price_per_kg "Giá trên mỗi kg (VND)"
        string source "Nguồn khảo sát (ví dụ: Chợ trung tâm)"
...
| `name` | `VARCHAR(100)` | NOT NULL | Tên cây trồng (ví dụ: Lúa Điện Biên, Cà phê Mường Ảng). |
| `variety` | `VARCHAR(100)` | NOT NULL | Giống cây trồng (ví dụ: IR64, Catimor). |
...
| `user_id` | `UUID` | FK -> `users(id)`, NOT NULL | Chủ sở hữu mảnh đất. |
| `crop_id` | `UUID` | FK -> `crops(id)`, NOT NULL | Loại cây trồng hiện tại trên mảnh đất. |
| `area_hectares`| `DOUBLE PRECISION`| NOT NULL | Diện tích canh tác tính bằng hecta. |
| `location_lat` | `DOUBLE PRECISION`| NOT NULL | Vĩ độ GPS. |
| `location_lng` | `DOUBLE PRECISION`| NOT NULL | Kinh độ GPS. |
...
Lưu trữ thông tin lịch sử phát hiện bệnh từ hình ảnh do nông dân chụp.
...
| `image_url` | `VARCHAR(255)` | NOT NULL | Đường dẫn ảnh lưu trữ trên Cloud Storage/Server. |
...
| `treatment_measures`| `TEXT`| NOT NULL | Biện pháp điều trị gợi ý. |
| `status` | `VARCHAR(20)` | Default `'active'` | Trạng thái ổ dịch: `active` (đang xảy ra), `resolved` (đã xử lý xong). |
| `created_at` | `TIMESTAMP` | Default `NOW()` | Thời điểm phát hiện bệnh. |
| `resolved_at` | `TIMESTAMP` | NULL | Thời điểm xử lý xong bệnh. |
...
| `optimal_harvest_start`| `DATE`| NOT NULL | Ngày bắt đầu khuyến nghị thu hoạch. |
...
Lưu trữ thông tin giá cả nông sản biến động hàng ngày.
...
| `price_per_kg` | `DOUBLE PRECISION`| NOT NULL | Giá bán (VND/kg). |
| `source` | `VARCHAR(150)` | NOT NULL | Nguồn thông tin giá (chợ đầu mối, sở nông nghiệp...). |
